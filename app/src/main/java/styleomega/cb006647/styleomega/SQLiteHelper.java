...
import styleomega.cb006647.styleomega.Model.*;

public class SQLiteHelper extends SQLiteOpenHelper {

    static String DATABASE_NAME="OmegaBase";
    ...
    public void onCreate(SQLiteDatabase database) {
        ...
        database.execSQL(CREATE_TABLE); //execute SQL statement that's not insert or or
        ...
        CREATE_TABLE="CREATE TABLE IF NOT EXISTS "+TABLE_CONTACT+" ("
                +Table_Column_1_CONTACTID+" INTEGER AUTOINCREMENT, "
    ...
    public ArrayList<Product> getAllProductData(String type) {
        SQLiteDatabase db = this.getWritableDatabase();  //create or open a database that will be used for reading and writing.
        Cursor result = db.rawQuery("select * from "+TABLE_CLOTHEPRODUCTS+" WHERE "+Table_Column_2_CLOTHETYPE+"='"+type+"'",null);
        //A Cursor object will return all rows that the query returns.
        ArrayList<Product> mens = new ArrayList<>();

        while(result.moveToNext()){  //move to next in the query result

            Product men = new Product
                    (result.getInt(result.getColumnIndex(SQLiteHelper.Table_Column_1_CLOTHEID))
    ...
    public boolean updateData(String emailholdervalue,String email,String password) {
        SQLiteDatabase db = this.getWritableDatabase(); //create or open a database that will be used for reading and writing.
        ContentValues contentValues = new ContentValues(); //Defines an object to contain the new values to insert
...
*/
    ...
    public String getDataForUser(String username) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("select * from "+TABLE_NAME,null); //A Cursor object will return all rows that the query returns.
        while(res.moveToNext()){//move to next raw in the query
...
*/
    ...
    public boolean addToCart(Product menItem, String userID, int quantity){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues cv =  new ContentValues(); //Defines an object to contain the new values to insert
